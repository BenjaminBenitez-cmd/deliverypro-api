...
Delivery.getMany = (company_id) => {
  return db.query(
    "WITH query_delivery AS ( SELECT dl.id, dl.client, dl.delivery_status, t.time_start, t.time_end, n.name AS delivery_day FROM delivery AS dl INNER JOIN times AS t ON dl.delivery_time = t.id INNER JOIN names_of_days AS n ON dl.delivery_day = n.id WHERE dl.delivery_company_id = $1 ) , query_clients AS ( SELECT client.id, client.first_name, client.last_name, client.phone_number, client.email, address.district, address.street FROM client INNER JOIN address ON address.client_id = client.id WHERE client.delivery_company_id = $1 ) SELECT query_delivery.*, query_clients.first_name, query_clients.last_name, query_clients.phone_number, query_clients.email, query_clients.district, query_clients.street FROM query_delivery INNER JOIN query_clients ON query_delivery.client = query_clients.id",
    [company_id]
  );
};
