...
Delivery.createOne = (
  ...
  return db.query(
    `INSERT INTO delivery (client, delivery_day, delivery_time, delivery_date, delivery_driver, delivery_company_id) values ($1, $2, $3, $4, $5, $6) returning *`,
    [
      client,
      delivery_day,
      delivery_time,
...
// DELETE A DELIVERY
Delivery.removeOne = (id, company_id) => {
  return db.query(
    `DELETE FROM delivery WHERE (id= $1 and delivery_company_id = $2) returning *`,
    [id, company_id]
  );
};
...
Delivery.updateVerification = (id, delivery_company_id) => {
  return db.query(
    "UPDATE delivery SET verified = true WHERE id = $ AND delivery_company_id = $2 returning *",
    [id, delivery_company_id]
  );
};
