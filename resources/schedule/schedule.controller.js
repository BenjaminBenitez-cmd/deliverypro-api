...
const getCompanySchedule = async (req, res) => {
    ...
    try {
        ...
        if(results.rows[0] === undefined) {
            return res.status(404).send({ status: 'not found', message: 'No active schedule' });
        }

        const days = await db.query("SELECT t.id, t.time_start, t.time_end, n.name, n.id AS name_of_day_id FROM times As t INNER JOIN names_of_days AS n ON t.name_of_day_id = n.id WHERE t.schedule_id = $1", 
        [results.rows[0].id]); 
...
const addCompanySchedule = async (req, res) => {
    ...
    try {
        const results = await db.query("INSERT INTO schedules (name, company_id) VALUES ($1, $2) returning *", [name, companyID]);

        if(results.rows[0] === undefined) {
            return res.status(400).send({ status: 'error', message: 'Unable to process request'});
        }
...
const addDayAndTime = async (req, res, next) => {
    ...
    try {
        ...
        const isValid = await db.query("SELECT company_id FROM schedules WHERE id = $1", [schedule_id]);
        //if the company_id does not exist, that means the request is not valid
        if(isValid.rows[0] === undefined || isValid.rows[0].company_id !== companyID) {
        ...
        if(time.rows[0] === undefined) {
            res.status(500).send({ status: 'error', message: 'There was an error with your request'});
        }
...
const updateTime = async (req, res) => {
    ...
    if(!req.body.time_start, !req.body.time_end) {
        return res.status(400).send({ status: 'error', message: 'Missing time parameters'});
    }

    const id = req.params.id;
    const {time_start, time_end } = req.body;
    try {
        const results = await db.query("UPDATE times SET time_start = $1, time_end = $2 WHERE id = $3 returning *", [time_start, time_end, id]);

        if(results.rows[0] === undefined) {
            return res.status(500).send({ status: 'error', message: 'There was an error'});
        }
