...
module.exports.signup = async (req, res) => {
  if (
    ...
    return res
      .status(400)
      .send({ message: "need email, company and password" });
  }
  ...
  try {
    ...
    const companyQuery = await db.query(
      "SELECT name FROM company WHERE LOWER(name) = LOWER($1)",
      [company]
    );
    if (companyQuery.rows[0] !== undefined)
      return res.status(400).json({
        status: "error",
        message: "Company, already added login instead",
      });

    const token = jwt.sign(
      { email, company, password, phone_number, name },
...
module.exports.signupAuthentication = async (req, res) => {
  ...
  try {
    const { email, company, password, phone_number, name } = await verifyToken(
      token
    );
    ...
    if (user.rows[0] !== undefined)
      return res
        .status(400)
        .send({ status: "error", message: "User with email already exists" });
...
module.exports.signin = async (req, res) => {
  if (!req.body.email || !req.body.password) {
    return res
      .status(400)
      .json({ status: "error", message: "Empty password or email" });
  }
  ...
  try {
    ...
    if (user.rows[0] === undefined)
      return res
        .status(401)
        .send({ status: "error", message: "No user with credentials" });
...
module.exports.registerForDriver = async (req, res) => {
  ...
  try {
    ...
    if (checkForUser.rows[0] == undefined)
      return res
        .status(400)
        .json({ status: "success", message: "No invite found" });
    //hash the password
    const hash = await hashPassword(password);
